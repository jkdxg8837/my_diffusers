...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--init_config",
        type=str,
        default="../config/gradient.yaml",
        help="Path to the LoRA-One init config yaml (e.g. gradient.yaml or svd.yaml).",
    )
...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            if args.full_finetune:
                # Save trainable weights directly for full finetune mode
                for model in models:
                    if isinstance(unwrap_model(model), type(unwrap_model(transformer))):
                        model = unwrap_model(model)
                        trainable_state = {name: param.data.clone() for name, param in model.named_parameters() if param.requires_grad}
                        torch.save(trainable_state, os.path.join(output_dir, "trainable_weights.pt"))
                    if weights:
                        weights.pop()
            else:
    ...
    def load_model_hook(models, input_dir):
        if args.full_finetune:
            # Load trainable weights for full finetune mode
            while len(models) > 0:
                model = models.pop()
                if isinstance(unwrap_model(model), type(unwrap_model(transformer))):
                    ...
                    if os.path.exists(weights_path):
                        trainable_state = torch.load(weights_path, map_location="cpu")
                        transformer_.load_state_dict(trainable_state, strict=False)
            return
    ...
    if args.full_finetune:
        # Full finetune mode: unfreeze target_modules weights directly on the transformer
        # set weight decay and learning rate to 1e-6 and 1e-7
        ...
        named_grads = torch.load("/home/j/jiayang_gu/workspace/diffusers/slurm/dog-lora-one-gamma9/raw_gradient_averages_4.pt")
        additional_info = {
            "named_grads": named_grads,}
        torch.save(named_grads, args.output_dir + "/named_grads.pth")
    ...
    transformer_parameters_with_lr = {"params": transformer_lora_parameters, "lr": args.learning_rate, "reg": 0}
    params_to_optimize = [transformer_parameters_with_lr]
    if not args.baseline and not args.full_finetune:
    ...
    # Optimizer creation
    if not (args.optimizer.lower() == "prodigy" or args.optimizer.lower() == "adamw" or args.optimizer.lower() == "adamwr"):
        logger.warning(
    ...
    # if args.optimizer.lower() == "adamwr":
    #     optimizer_class = AdamWr
    #     optimizer = optimizer_class(
    #         params_to_optimize,
    #         betas=(args.adam_beta1, args.adam_beta2),
    #         weight_decay=args.adam_weight_decay,
    ...
    # Re-seed RNG before training loop to ensure identical batch order across methods.
    # This is critical because LoRA-One's gradient estimation consumes RNG state.
    if args.seed is not None:
        set_seed(args.seed)

    # Open loss log file
    loss_log_path = os.path.join(args.output_dir, "loss_curve.jsonl")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                loss_val = loss.item()
                print(f"Step {global_step}: Loss = {loss_val}")
                # Log per-step loss to jsonl
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step in save_ckpt_list:
                        ...
                        step_gradients = {}
                        # 遍历 transformer 的参数
                        # 注意：我们遍历 'transformer' 本身，而不是 accelerator 包装后的版本
                        # 'accelerator' 会自动处理底层模型的梯度
                        for name, param in transformer.named_parameters():
                            if param.grad is not None:
                                # 必须使用 .detach().clone() 来创建一个独立的副本
                                # 否则它将在 zero_grad() 时被清除
                                
                                step_gradients[f"transformer_{name}"] = param.grad.detach().clone()
                        
                        torch.save(step_gradients, f"{args.output_dir}/grads_step_{global_step}.pt")
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
    ...
    if accelerator.is_main_process:
        ...
        if args.full_finetune:
            # Save trainable weights directly for full finetune
            trainable_state = {name: param.data.clone() for name, param in transformer.named_parameters() if param.requires_grad}
            torch.save(trainable_state, os.path.join(args.output_dir, "trainable_weights.pt"))
        else:
            if args.upcast_before_saving:
