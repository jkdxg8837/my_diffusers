...
def reinit_lora_from_fft(model, init_config, fft_state_dict, pretrained_state_dict):
    ...
    for name, module in tqdm(
        ...
        if isinstance(module, LoraLayer):
            # Compute and store the delta for this module
            weight_name = name + '.weight'
            if weight_name in fft_state_dict and weight_name in pretrained_state_dict:
                delta = (fft_state_dict[weight_name] - pretrained_state_dict[weight_name]).cpu()
                module_deltas[name] = delta

            if_init = reinit_fft_weights(name, module, init_config, additional_info)
            if if_init:
                inited_modules.append(name)

    # Save all module deltas to a single file
    output_dir = init_config.get("output_dir", ".")
    delta_save_path = os.path.join(output_dir, "module_deltas.pt")
    torch.save(module_deltas, delta_save_path)
    print(f"Saved {len(module_deltas)} module deltas to {delta_save_path}")
