...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Comma-separated list of specific steps at which to save checkpoints. "
            "E.g. '0,10,100,200,300,400,500'. Step 0 saves before any training. "
            "When set, overrides --checkpointing_steps."
        ),
    )
    parser.add_argument(
...
def main(args):
    ...
    # Parse save_at_steps into a set (if provided)
    save_at_steps_set = None
    if args.save_at_steps is not None:
        save_at_steps_set = set(int(s.strip()) for s in args.save_at_steps.split(","))
    ...
    # Save step-0 checkpoint (before any training) if requested
    if save_at_steps_set is not None and 0 in save_at_steps_set and initial_global_step == 0:
        if accelerator.is_main_process or is_fsdp:
            save_path = os.path.join(args.output_dir, "checkpoint-0")
            accelerator.save_state(save_path)
            logger.info(f"Saved step-0 state to {save_path}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process or is_fsdp:
                    should_save = (
                        global_step in save_at_steps_set
                        if save_at_steps_set is not None
                        else global_step % args.checkpointing_steps == 0
                    )
                    if should_save:
